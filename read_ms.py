...
def read_ms(ms, num_vis, res_arcmin, chunks=50000, channel=0, field_id=0):
    ...
    try:
        ...
        for i, ds in enumerate(datasets):
            ...
            if int(field_id) == int(ds.FIELD_ID):
                ...
                raw_vis = read_np_array(
                    ds.DATA.data[indices, channel, pol], "DATA", dtype=np.complex64
                )

                corrected_vis = read_np_array(
                    ds.CORRECTED_DATA.data[indices, channel, pol], "DATA", dtype=np.complex64
                )
        ...
        logger.info("Max vis {}".format(np.max(np.abs(corrected_vis))))
    ...
    return u_arr, v_arr, w_arr, frequency, raw_vis, corrected_vis, hdr, timestamp, rms_arr
